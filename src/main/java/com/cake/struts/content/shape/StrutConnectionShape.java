...
/**
 * Client-side strut connection shape for ray-casting and outline rendering.
 * Stored in world-space coordinates.
 */
public class StrutConnectionShape {

    private static final double INFLATE_PIXELS = 0.0;
    private static final double TANGENT_EXTENSION_PIXELS = 0.0;
    ...
    public StrutConnectionShape(final Vec3 fromAttachment, final Vec3 toAttachment,
        ...
        this(List.of(fromAttachment, toAttachment), halfWidth, halfHeight);
    }

    public StrutConnectionShape(final List<Vec3> points,
                                final double halfWidth, final double halfHeight) {
    ...
    public static StrutConnectionShape cable(final Vec3 fromAttachment, final Vec3 toAttachment,
                                             final double halfWidth, final double halfHeight,
                                             final @Nullable CableStrutInfo renderInfo) {
        if (renderInfo == null) {
            return new StrutConnectionShape(fromAttachment, toAttachment, halfWidth, halfHeight);
        }

        final Vec3 delta = toAttachment.subtract(fromAttachment);
        final double spanLength = delta.length();
        if (spanLength <= MIN_LENGTH_SQR) {
            return new StrutConnectionShape(fromAttachment, toAttachment, halfWidth, halfHeight);
        }

        final List<Vec3> sampledPoints = CableStrutModelManipulator.sampleCurvePoints(fromAttachment, toAttachment, renderInfo, spanLength);
        return new StrutConnectionShape(sampledPoints, halfWidth, halfHeight);
    }

    /**
     * Returns the world-space intersection point of the given ray with this shape,
    ...
    public Vec3 intersect(final Vec3 rayFrom, final Vec3 rayTo) {
        ...
        for (int segmentIndex = 0; segmentIndex < segmentCount; segmentIndex++) {
            ...
            final double halfLength = segmentLength * 0.5 + TANGENT_EXTENSION_PIXELS;
    ...
    /**
     * Draws an outline around this strut connection, translated by {@code -camera}.
     */
    ...
    public void drawOutline(final PoseStack ms, final VertexConsumer vb, final Vec3 camera, final int color) {
        ...
        for (int segmentIndex = 0; segmentIndex < segmentCount; segmentIndex++) {
            final int startIndex = segmentIndex;
            final int endIndex = segmentIndex + 1;

            final Vec3 start = points.get(startIndex);
            final Vec3 end = points.get(endIndex);

            final List<Vec3> startCorners = getCorners(start, uAtVertex[startIndex], vAtVertex[startIndex], hw, hh);
